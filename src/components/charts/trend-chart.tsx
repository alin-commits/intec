...
export function TrendChart({ data }: TrendChartProps) {
  ...
  return (
    <div className="chart-wrap" aria-label="Evolución mensual de consultas web y telefónicas">
      <svg viewBox={`0 0 ${width} ${height}`} role="img" className="chart-svg">
        ...
        {data.map((item, index) => {
          ...
          const x = padding.left + (index / Math.max(1, data.length - 1)) * chartWidth;
          return (
            <text key={item.label} x={x} y={height - 12} textAnchor="middle" className="chart-label">
              {item.label}
            </text>
          );
        })}
      ...
      <div className="chart-legend">
        <span><i className="legend-dot legend-web" /> Web</span>
        <span><i className="legend-dot legend-phone" /> Telefónicas</span>
      </div>
    </div>
  );
}
