...
export function TicketFilters({ filters, departments, resultCount, onChange }: TicketFiltersProps) {
  ...
  const defaults = blankTicketFilters();
  const hasActiveFilters = filters.query !== defaults.query
    || filters.status !== defaults.status
    || filters.priority !== defaults.priority
    || filters.category !== defaults.category
    || filters.department !== defaults.department
    || filters.dateFrom !== defaults.dateFrom
    || filters.dateTo !== defaults.dateTo
    || filters.onlyOpen !== defaults.onlyOpen;

  return (
    <CollapsibleFilters
      hasActiveFilters={hasActiveFilters}
      onClear={() => onChange(blankTicketFilters())}
