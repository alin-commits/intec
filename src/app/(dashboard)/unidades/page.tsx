    ...
    <div className="page-stack">
      ...
      <section className="units-grid">
        {businessUnits.map((unit) => {
          ...
          return <article className="panel unit-summary" key={unit.id}><div className="unit-accent" style={{ background: unit.accent }} /><h3>{unit.name}</h3><p>{(row?.web ?? 0) + (row?.phone ?? 0)} consultas · {row?.leads ?? 0} leads · {formatPercent(conversion)} conversión</p></article>;
        })}
      </section>
    </div>
  );
