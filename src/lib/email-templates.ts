...
<html lang="es">
  <body style="margin:0;padding:32px 16px;background:#f5f7fb;font-family:Segoe UI,Arial,sans-serif;color:#0f172a;">
    <table role="presentation" width="100%" style="max-width:480px;margin:0 auto;background:#ffffff;border-radius:14px;border:1px solid #e8ebf2;overflow:hidden;">
      <tr><td style="padding:28px 32px 8px;">
        <p style="margin:0;font-size:13px;font-weight:700;letter-spacing:.04em;text-transform:uppercase;color:#1d4ed8;">Intec Commercial Hub</p>
...
export function buildInviteEmail(fullName: string, actionLink: string): { subject: string; html: string } {
  const body = `
    ...
    ${button(actionLink, "Activar mi cuenta")}
...
export function buildResetPasswordEmail(actionLink: string): { subject: string; html: string } {
  const body = `
    ...
    ${button(actionLink, "Restablecer contraseña")}
